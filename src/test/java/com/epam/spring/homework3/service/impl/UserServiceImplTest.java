...
import static com.epam.spring.homework3.testUtils.TestUserDataUtil.*;
...
public class UserServiceImplTest {
    ...
    void testUpdateUser() {
        ...
        //when
        UserDTO updatedUser = userService.updateUser(MOCK_ID, userBody);
    ...
    void testUpdateUserThrowsExceptionIfUserDoesntExist() {
        //given
        UserDTO userBody = UserDTO.builder().email(MOCK_EMAIL).build();

        //when
        when(userRepository.findById(MOCK_ID)).thenReturn(Optional.empty());

        //then
        assertThrows(EntityNotFoundException.class, () -> userService.updateUser(MOCK_ID, userBody));
    }
